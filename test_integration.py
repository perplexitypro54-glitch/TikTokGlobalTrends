...
def test_basic_imports():
    ...
    try:
        ...
        assert NicheType.FITNESS == 'FITNESS'
        assert TrendDirection.UPWARD == 'UPWARD'
...
def test_keyword_extraction():
    ...
    try:
        ...
        text = "Amazing workout routine fitness gym exercise health"
        keywords = processor.extract_keywords(text, max_keywords=5)
        
        assert len(keywords) > 0, "Should extract some keywords"
        assert "amazing" not in keywords, "Should filter stop words"
...
def test_hashtag_processing():
    ...
    try:
        ...
        assert hashtag.trend_direction == TrendDirection.UPWARD, "Should detect upward trend"
